...
/**
 * Performance test for the Casa entity.
 */
class CasaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Casa entity")
        ...
        .repeat(2) {
            ...
            .post("/api/casas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "direccion":"SAMPLE_TEXT", "color":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_casa_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created casa")
                .get("${new_casa_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created casa")
            .delete("${new_casa_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
