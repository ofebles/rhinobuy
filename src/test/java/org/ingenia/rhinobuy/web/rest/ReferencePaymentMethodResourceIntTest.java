...
public class ReferencePaymentMethodResourceIntTest {
    ...
    public void initTest() {
        referencePaymentMethodSearchRepository.deleteAll();
    ...
    public void createReferencePaymentMethod() throws Exception {
        ...
        // Validate the ReferencePaymentMethod in ElasticSearch
        ReferencePaymentMethod referencePaymentMethodEs = referencePaymentMethodSearchRepository.findOne(testReferencePaymentMethod.getId());
        assertThat(referencePaymentMethodEs).isEqualToComparingFieldByField(testReferencePaymentMethod);
    }
    ...
    public void deleteReferencePaymentMethod() throws Exception {
        ...
        boolean referencePaymentMethodExistsInEs = referencePaymentMethodSearchRepository.exists(referencePaymentMethod.getId());
        assertThat(referencePaymentMethodExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchReferencePaymentMethod() throws Exception {
        // Initialize the database
        referencePaymentMethodService.save(referencePaymentMethod);

        // Search the referencePaymentMethod
        restReferencePaymentMethodMockMvc.perform(get("/api/_search/reference-payment-methods?query=id:" + referencePaymentMethod.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(referencePaymentMethod.getId().intValue())))
            .andExpect(jsonPath("$.[*].description").value(hasItem(DEFAULT_DESCRIPTION.toString())));
