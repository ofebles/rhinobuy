...
public class OrderItemStatusCodeResourceIntTest {
    ...
    public void initTest() {
        orderItemStatusCodeSearchRepository.deleteAll();
    ...
    public void createOrderItemStatusCode() throws Exception {
        ...
        // Validate the OrderItemStatusCode in ElasticSearch
        OrderItemStatusCode orderItemStatusCodeEs = orderItemStatusCodeSearchRepository.findOne(testOrderItemStatusCode.getId());
        assertThat(orderItemStatusCodeEs).isEqualToComparingFieldByField(testOrderItemStatusCode);
    }
    ...
    public void deleteOrderItemStatusCode() throws Exception {
        ...
        boolean orderItemStatusCodeExistsInEs = orderItemStatusCodeSearchRepository.exists(orderItemStatusCode.getId());
        assertThat(orderItemStatusCodeExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchOrderItemStatusCode() throws Exception {
        // Initialize the database
        orderItemStatusCodeService.save(orderItemStatusCode);

        // Search the orderItemStatusCode
        restOrderItemStatusCodeMockMvc.perform(get("/api/_search/order-item-status-codes?query=id:" + orderItemStatusCode.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(orderItemStatusCode.getId().intValue())))
            .andExpect(jsonPath("$.[*].description").value(hasItem(DEFAULT_DESCRIPTION.toString())));
